...
export class LineItemController {
  ...
  async importList(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
      ...
      if (!Array.isArray(body)) {
        return {
          status: 400,
          jsonBody: { message: 'Request body must be an array of line items' },
        };
      }
  ...
  async getMany(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
      ...
      const query: QueryLineItemDto = {
        ...
        taxable:
          url.searchParams.get('taxable') === 'true'
            ? true
            : url.searchParams.get('taxable') === 'false'
              ? false
              : undefined,
        warranty:
          url.searchParams.get('warranty') === 'true'
            ? true
  ...
  async getTaxable(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
      ...
      if (taxableParam !== 'true' && taxableParam !== 'false') {
        return {
          status: 400,
          jsonBody: { message: "Invalid taxable parameter. Must be 'true' or 'false'" },
        };
      }
  ...
  async getWithWarranty(
    ...
    try {
      ...
      if (warrantyParam !== 'true' && warrantyParam !== 'false') {
        return {
          status: 400,
          jsonBody: { message: "Invalid warranty parameter. Must be 'true' or 'false'" },
        };
      }
  ...
  async getByServiceTypeIdAndTypeAndUnitPrice(
    request: HttpRequest,
    context: InvocationContext
  ): Promise<HttpResponseInit> {
    try {
      ...
      if (!unitPrice || isNaN(Number(unitPrice))) {
        return {
          status: 400,
          jsonBody: { message: 'Unit price parameter is required and must be a valid number' },
        };
      }
  ...
  // GET /line-items/warranty-filter/{serviceTypeId}/{type}/{vehicleId}
  async getByServiceTypeAndTypeAndVehicleIdWithWarrantyTrue(
    request: HttpRequest,
    context: InvocationContext
  ): Promise<HttpResponseInit> {
    try {
      ...
      const lineItems = await lineItemService.findByServiceTypeAndTypeAndVehicleIdWithWarrantyTrue(
        serviceTypeId,
        type,
        vehicleId
      );
...
app.http('GetLineItemsByServiceTypeAndTypeAndVehicleIdWithWarrantyTrue', {
  methods: ['GET'],

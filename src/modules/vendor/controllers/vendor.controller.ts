...
export class VendorController {
  ...
  // GET /vendors/count?q=&status=&type=
  async getCount(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
      ...
      context.error('vendor.getCount error', err);
      return this.toError(err);
    }
  }
...
app.http('GetVendorsCount', {
  methods: ['GET'],
  route: 'v1/count/vendors',
  authLevel: 'function',
  handler: (req, ctx) => vendorController.getCount(req, ctx),
});
